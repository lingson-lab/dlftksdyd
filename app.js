...
function render(){result=C.analyze(rows,{month:$('month').value,excludeTest:$('exclude').checked,lockerTradeFallback:$('locker-trade-fallback').checked});const s=result.stats;
...
function renderBilling(){
 ...
 const audit=groups.lockerAudit;
 $('locker-audit').innerHTML=`<p>락커·신발장 인식 ${audit.matched}건 · 합산에 반영 ${audit.included}건${audit.tradeFallback?' · 거래월 대체 '+audit.tradeFallback+'건':''}</p>${!audit.matched?'<p>원본 파일에서 지정된 5개 명칭을 찾지 못했습니다. 등록명·수강소분류·수강대분류를 확인해 주세요.</p>':''}${audit.issues.length?`<details><summary>제외·확인 내역 ${audit.issues.length}건</summary>${audit.issues.map(r=>`<div class="product-row"><span>${esc(r.dong)}동 ${esc(r.ho)}호 · ${esc(r.name)} · ${esc(r.product)}</span><span>${esc(r.reason)}</span></div>`).join('')}</details>`:''}`;
...
$('locker-trade-fallback').onchange=render;$('month').onchange=render;$('exclude').onchange=render;
